...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        ### PARAMETERS ###
        self.map_name = "E1_out2_refined1"#"traj_race_cl.csv"  # Name of csv with trajectory points
        ...
        map_path = os.path.abspath(os.path.join('src', "f1tenth-software-stack",'csv_data' + '/' + self.map_name + '.csv'))
        with open(map_path, 'r') as f:
             lines = (line for line in f if not line.startswith('#'))
             data = np.loadtxt(lines, delimiter=',')
             self.goal_points = data[:, 0:2]
        ...
        max_derivatives_val = np.max(self.derivatives)
        if max_derivatives_val == 0:
            self.derivatives = np.zeros_like(self.derivatives)
        else:
            self.derivatives = np.where(self.derivatives / max_derivatives_val > 1, 1 ,self.derivatives / max_derivatives_val)# - 5) )
